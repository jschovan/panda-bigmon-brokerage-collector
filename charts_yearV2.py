...
from monthlyDBV2 import monthlyDBV2
...
SQLS = [
        "select category, sum(jobdefCount) nums from monthlylogV2 where logMonth > '%s' group by category order by category",
        "select site, sum(jobdefCount) nums from monthlylogV2 where category='A' and logMonth > '%s' group by site order by nums DESC",
        "select cloud, sum(jobdefCount) nums from monthlylogV2 where category='B' and logMonth > '%s' group by cloud order by nums DESC",
        "select site, sum(jobdefCount) nums from monthlylogV2 where category='C' and logMonth > '%s' group by site order by nums DESC",
        "select cloud, sum(jobdefCount) nums from monthlylogV2 where category='C' and logMonth > '%s' group by cloud order by nums DESC",
        "select site, sum(jobdefCount) nums from monthlylogV2 where category='E' and logMonth > '%s' group by site order by nums DESC",
        "select cloud, sum(jobdefCount) nums from monthlylogV2 where category='E' and logMonth > '%s' group by cloud order by nums DESC"
        ]
...
def write_document(document, FILENAME='yearlyV2.html'):
    of = open(FILENAME, 'w')
    print >>of, document
    of.close()
